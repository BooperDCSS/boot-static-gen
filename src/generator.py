...
def generate_page(
    ...
    print(
        f"Generating page from {os.path.relpath(from_path, ROOT_DIRECTORY)} to {os.path.relpath(dest_path, ROOT_DIRECTORY)} using {os.path.relpath(template_path, ROOT_DIRECTORY)}"
    )

    _, ext = os.path.splitext(from_path)
    if ext != ".md":
        raise ValueError(
            f"Invalid file type: {ext}. This program converts .md files. Stopping conversion."
        )
    ...
    new_html = template_html.replace("{{ Content }}", html_content).replace(
        "{{ Title }}", html_title
    )
    final_html = new_html.replace('href="/', f'href="{basepath}').replace(
        'src="/', f'src="{basepath}'
    )
...
def generate_pages_recursive(
    ...
    if not os.path.exists(from_path):
        raise Exception(
            "No content found. Place your .md files inside `ROOT_DIRECTORY/content`."
        )
    if not os.path.exists(template_path):
        raise Exception("HTML template missing from root directory.")
    ...
    if len(content_dir_list) < 1:
        raise ValueError(
            "Content directory is empty; add your .md files to the `content` directory."
        )
    for item in content_dir_list:
        ...
        if os.path.isdir(file_path):
            generate_pages_recursive(
                file_path, template_path, os.path.join(dest_path, item), basepath
            )

        elif os.path.isfile(file_path):
            file_name, ext = os.path.splitext(item)
            target_path = os.path.join(dest_path, file_name + ".html")
            generate_page(file_path, template_path, target_path, basepath)
...
def static_to_public(source: str, target: str) -> None:
    try:
        ...
        if not os.path.exists(target):
            os.mkdir(target)
            logger.info(
                f"Directory created: `{os.path.relpath(target, ROOT_DIRECTORY)}`"
            )
    except Exception:
        logger.exception(
